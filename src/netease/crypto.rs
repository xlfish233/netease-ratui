...
static RSA_PUBLIC_KEY: Lazy<RsaPublicKey> =
    Lazy::new(|| RsaPublicKey::from_public_key_pem(PUBLIC_KEY_PEM).expect("bad PUBLIC_KEY_PEM"));
...
fn aes_128_cbc_encrypt_base64(pt: &[u8], key: &[u8], iv: &[u8]) -> String {
    let mut buf = pt.to_vec();
    let msg_len = buf.len();
    buf.resize(msg_len + 16, 0);
    let ct = Aes128CbcEnc::new(key.into(), iv.into())
        .encrypt_padded_mut::<Pkcs7>(&mut buf, msg_len)
        .expect("cbc pkcs7 pad");
    base64::engine::general_purpose::STANDARD.encode(ct)
}

fn aes_128_ecb_encrypt_hex_upper(pt: &[u8], key: &[u8]) -> String {
    let mut buf = pt.to_vec();
    let msg_len = buf.len();
    buf.resize(msg_len + 16, 0);
    let ct = Aes128EcbEnc::new(key.into())
        .encrypt_padded_mut::<Pkcs7>(&mut buf, msg_len)
        .expect("ecb pkcs7 pad");
    hex::encode_upper(ct)
}
...
pub fn weapi(data: &Value) -> WeapiForm {
    let text = serde_json::to_string(data).expect("serialize weapi json");
    ...
    WeapiForm { params, enc_sec_key }
}

pub fn linuxapi(data: &Value) -> LinuxapiForm {
    let text = serde_json::to_string(data).expect("serialize linuxapi json");
    let eparams = aes_128_ecb_encrypt_hex_upper(text.as_bytes(), LINUXAPI_KEY.as_bytes());
    LinuxapiForm { eparams }
}

pub fn eapi(uri: &str, data: &Value) -> EapiForm {
    let text = serde_json::to_string(data).expect("serialize eapi json");
