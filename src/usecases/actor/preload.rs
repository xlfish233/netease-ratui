use super::playlist_tracks::PlaylistTracksLoad;
...
#[derive(Default)]
pub(super) struct PreloadManager {
    generation: u64,
...
impl PreloadManager {
    pub(super) fn owns_req(&self, req_id_evt: u64) -> bool {
        self.pending
            .get(&req_id_evt)
            .is_some_and(|(generation, _)| *generation == self.generation)
    }

    pub(super) fn reset(&mut self, app: &mut App) {
    ...
    pub(super) async fn start_for_playlists(
        &mut self,
        app: &mut App,
    ...
    pub(super) fn cancel_playlist(&mut self, app: &mut App, playlist_id: i64) {
    ...
    pub(super) async fn on_playlist_track_ids(
        &mut self,
        app: &mut App,
        tx_netease_lo: &mpsc::Sender<NeteaseCommand>,
        req_id: &mut u64,
...
pub(super) fn update_preload_summary(app: &mut App) {
